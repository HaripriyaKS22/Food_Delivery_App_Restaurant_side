...
import 'package:homefoo_resto_hari/add_product.dart';
import 'package:homefoo_resto_hari/api.dart';
import 'package:homefoo_resto_hari/login.dart';
import 'package:homefoo_resto_hari/update_product.dart';
  ...
  Future<void> fetchrestaurantdetails() async {
    try {
      ...
      final response = await http.get(Uri.parse(
          "$api/HOMFOO-restaurant/restaurant-details/$userId/"));
      print("aaaaapppppppppppiiiiiiiiiiiiiiiiiiiii${response.body}");
      if (response.statusCode == 200) {
        final Map<String, dynamic> responseData = jsonDecode(response.body);
        final restData = responseData['data'];
        List<Map<String, dynamic>> categoriesList = [];

        setState(() {
          print(
              "reeeeeeeesssssssssssssssttttttttttttttttttttttttttttt${restData['is_open']}");
          if (restData['is_open'] == 'Open') {
            isSwitched = true;
          } else {
            isSwitched = false;
          }
  ...
  Future<void> deleteProduct(int pId) async {
    ...
    try {
      final response = await http.delete(
        Uri.parse('${api}/HOMFOO-restaurant/HOMFOO-shop-product-delete/$pId/'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': '$token',
        },
      );
      print("uuuuuuuuuuuuuuuuuuuuuuuuuu ${api}/HOMFOO-restaurant/HOMFOO-shop-product-delete/$pId/");
...
// sharedpreference
  ...
  Future<void> _initData() async {
    ...
    _startTimer();
    // fetchCategories();
    await fetchProducts(userId); // Use userId after getting the value
    toggleStates = List<bool>.filled(filteredProducts.length, false);
  }
...
//product
  Future<void> fetchProducts(String? userId) async {
    ...
    try {
      var response = await http.post(Uri.parse('$api/HOMFOO-restaurant/HOMFOO-shop-products/'), headers: {
        'Authorization': '$token',
      }, body: {
        'token': token,
      });
      ...
      if (response.statusCode == 200) {
        ...
        for (var productData in productsData) {
          ...
          toggleStatesList.add(productData['is_active'] == 'Active');

          print("toggleStatesListtttttttttttt: $toggleStatesList");
        }
  ...
  Future<void> deactivaterestaurants() async {
    try {
      print('dgggggggggggggggggggg');
      var userId = await getUserIdFromPrefs();
      var token = await gettokenFromPrefs();
      print(
          'https://majority-double-continues-dame.trycloudflare.com/HOMFOO-restaurant/resturent_deactivate/$userId/');
      ...
      if (response.statusCode == 200) {
        print("restaurant deactivated successfully");
        ...
        print("Failed to deactivate ${response.body}");
      }
    } catch (error) {
      print("Error deactivating restaurant: $error");
    }
  }

  Future<void> activaterestaurants() async {
    try {
      print('dgggggggggggggggggggg');
      var userId = await getUserIdFromPrefs();
      var token = await gettokenFromPrefs();
      print(
          'https://majority-double-continues-dame.trycloudflare.com/HOMFOO-restaurant/resturent_activate/$userId/');
...
//banner
  ...
  //           String imageUrl = "https://majority-double-continues-dame.trycloudflare.com${categoryData['image']}";
  ...
  Future<void> getcategories() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          catee = productsData.map<Map<String, dynamic>>((data) {
            String imageUrl =
                "$api${data['image']}";
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      image: DecorationImage(
                                        image:
                                            NetworkImage(catee[index]['image']),
                                        fit: BoxFit.cover,
                                      ),
                                  ...
                                  child: Switch(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        ...
                                        // Update the product's is_active status based on the switch value
                                        filteredProducts[index]['is_active'] =
