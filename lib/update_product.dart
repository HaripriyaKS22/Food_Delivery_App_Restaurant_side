...
import 'package:homefoo_resto_hari/home.dart';
...
Future<void> fetchProducts(var Id) async 
  {
print('${api}/HOMFOO-restaurant/HOMFOO-shop-product-update/${widget.id}');
    final token = await gettokenFromPrefs();
    print("8888888888888888888888888888888888'${api}/HOMFOO-restaurant/HOMFOO-shop-product-update/${widget.id}/'");
    try {
     var response = await http.get(Uri.parse('${api}/HOMFOO-restaurant/HOMFOO-shop-product-update/${widget.id}/'));
  ...
  selectedCategoryId = productsData['category'].toString();
       setState(() {
    imageUrl1 = "${api}${productsData['image1']}";
    imageUrl2 = "${api}${productsData['image2']}";
    imageUrl3 = "${api}${productsData['image3']}";
  });
      } else {
  ...
  try {
    final response = await http.get(Uri.parse("https://majority-double-continues-dame.trycloudflare.com/categories/"));
...
void updateItem(
  ...
  try {
    var request = http.MultipartRequest('PUT', Uri.parse('$api/HOMFOO-restaurant/HOMFOO-shop-product-update/${widget.id}/')); // Assuming the URL format for updating data is like 'https://example.com/api/resource/id'
              ...
              child: TextButton(
                onPressed: () async {
                 token = await gettokenFromPrefs();
                  ...
                  updateItem(name.text,price.text,selectedImage,selectedImage2,selectedImage3,Offers.text,description.text,selectedCategoryId!,scaffoldContext,token!);
